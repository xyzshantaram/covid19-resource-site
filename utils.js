const App = {
  ...
  master:
    "https://docs.google.com/spreadsheets/d/16ebrAnBatGm69NTh0o1L8Nlnu4PoqIUfHZ0PtagXFLE/edit#gid=1644224808",
};
...
function showErrorDialog(msg) {
  ...
  setModalContent(
    msg,
    `<i class="fas fa-exclamation-triangle fs-4 mt-1 mb-1"></i>`,
    "Error",
    false,
  );
  Modal.show();
...
function setModalContent(
  ...
  document.getElementById("reusable-modal-content").innerHTML =
    `${eltString} ${content}`;
  if (isDismissable) {
    document.getElementById("reusable-modal-footer").innerHTML =
...
let isUnverified = (_item) => {
  ...
  return !(Boolean(item) && Boolean(item.trim())) ||
    item.trim().toLocaleLowerCase() === "retry";
};
